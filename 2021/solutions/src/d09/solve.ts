...
export async function main() {
  let s: string = await util.loadFile('/src/d09/input.txt')
  let t: number[][] = s.trim().split('\n').map(x => x.trim().split('').map(y => Number(y)))
  let N = t.length
  let M = t[0].length
  let NM = N * M
  ...
  function neighs(k: number): number[] {
  ...
  let seen: boolean[] = Array(NM).fill(false)

  function bfs(k: number): number {
    let count = 1;
    for (let neigh of neighs(k)) {
