...
func walk(s string, m map[point]int, val int, f func(p point, v int)) {
...
func p1(input string) {
	...
	walk(line[1], m, 2,
		func(p point, v int) {
			if v|1 == 0 {
				fmt.Println("Self crossing")
			} else {
				dist := abs(p.x) + abs(p.y)
				if dist < bestDistance {
					bestDistance = dist
					bestPiont = p
					fmt.Println("Found better point", bestPiont)
				}
			}
		})
	fmt.Println(bestDistance, bestPiont)
}

func walk2(s string, m map[point]int, addval bool, f func(p point, v1, v2 int)) {
	steps := strings.Split(s, ",")
	p := point{}
	counter := 0
	for _, step := range steps {
...
func p2(input string) {
	...
	m := map[point]int{}
	walk2(line[0], m, true, nil)

	bestDistance := math.MaxInt32
	bestPiont := point{0, 0}
	walk2(line[1], m, false,
		func(p point, v1, v2 int) {
			dist := v1 + v2
			if dist < bestDistance {
				bestDistance = dist
				bestPiont = p
